                ...
                <table class="table table-hover table-bordered table-stripped" id="example2" style="text-align: center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>No Attachment</td>
                            @else
                            <td><a href="{{ url('storage/filename/'.$wish->filename) }}" target="_blank" download>Download</a></td>
                            @endif
                            <td>
                                @if($user->hasRole('Admin'))
                                    @if($wish->status_id == '1') 
                                    <a href="{{route('wishes.nextstep2', $wish)}}" onclick="return confirm('Apakah anda ingin update status dokumen ini?')" class="btn btn-info btn-xs" method="post">
                                        Next Step
                                    </a>
                                    @elseif($wish->status_id == '2')
